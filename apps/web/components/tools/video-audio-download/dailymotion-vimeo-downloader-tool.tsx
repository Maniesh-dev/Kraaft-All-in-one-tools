...
import { VideoCamera, Download, Link, Spinner, FileVideo, FileImage, Warning, ArrowClockwise } from "@phosphor-icons/react";
...
export function DailymotionVimeoDownloaderTool() {
  ...
  const handleFetchInfo = async () => {
    if (!url) { toast.error("Please enter a Vimeo or Dailymotion URL"); return; }
    
    const isVimeo = url.match(/vimeo\.com\//);
    const isDailymotion = url.match(/(?:dailymotion\.com\/video\/|dai\.ly\/)/);
    
    if (!isVimeo && !isDailymotion) {
      toast.error("Please enter a valid Vimeo or Dailymotion URL"); return;
    }

    setLoading(true); setMediaInfo(null); setErrorMsg(null);
    try {
      const res = await fetch("/api/video-audio-download/dailymotion-vimeo", {
  ...
  const handleDownload = (mediaUrl: string, type: "video" | "image") => {
    const fn = `${mediaInfo?.platform || 'video'}-${type}-${Date.now()}`;
  ...
  const getGradient = (platform?: "vimeo" | "dailymotion") => {
    if (platform === "vimeo") return "from-[#1AB7EA] via-[#0D8BD9] to-[#0A66C2]";
    if (platform === "dailymotion") return "from-[#0066DC] via-[#000000] to-[#FF0044]";
    return "from-[#1AB7EA] to-[#FF0044]"; // Mixed fallback
  };

  const getPlatformName = (platform?: "vimeo" | "dailymotion") => {
    if (platform === "vimeo") return "Vimeo";
    if (platform === "dailymotion") return "Dailymotion";
    ...
    <div className="space-y-6">
      <Card className="border-border/60 shadow-sm overflow-hidden">
        ...
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <VideoCamera className="h-6 w-6 text-[#1AB7EA]" weight="bold" />Vimeo & Dailymotion Downloader
          </CardTitle>
          <CardDescription>Download videos from Vimeo and Dailymotion by pasting the video link below.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              <Link className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input placeholder="https://vimeo.com/... or https://dai.ly/..." value={url}
            ...
            <Button onClick={handleFetchInfo} disabled={loading || !url}
              className={`bg-gradient-to-r ${getGradient()} text-white hover:opacity-90 transition-opacity border-0`}>
          ...
          <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground bg-muted/30 p-2 rounded-lg border border-border/40">
            <Warning size={14} className="text-amber-500 shrink-0" /><span>Works for public videos only. Private or password-protected videos are not supported.</span>
          </div>
        </CardContent>
      </Card>
      ...
      {mediaInfo && (
        <Card className="animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-hidden border-border/60">
          <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${getGradient(mediaInfo.platform)}`} />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
            <div className="relative aspect-video bg-black flex items-center justify-center p-0">
              {mediaInfo.type === "video" && (selectedQuality || mediaInfo.videoUrl) ? (
                <><video src={`/api/video-audio-download/proxy?url=${encodeURIComponent(selectedQuality || mediaInfo.videoUrl!)}&filename=preview`}
                ...
                <><img src={`/api/video-audio-download/proxy?url=${encodeURIComponent(mediaInfo.imageUrl)}&filename=preview`}
                  alt={`${getPlatformName(mediaInfo.platform)} preview`} className="w-full h-full object-contain" />
            ...
            <div className="p-6 flex flex-col justify-between">
              <div className="space-y-4">
                ...
                <div className="flex justify-between text-sm py-2 border-b border-border/40">
                  <span className="text-muted-foreground">Source</span><span className="font-medium">{getPlatformName(mediaInfo.platform)}</span>
                </div>
                
                {mediaInfo.type === "video" && mediaInfo.qualities && mediaInfo.qualities.length > 0 && (
                  <div className="pt-2">
                    <label className="text-sm font-medium text-muted-foreground mb-2 block">Select Quality</label>
                    <Select value={selectedQuality} onValueChange={setSelectedQuality}>
                      ...
                      <SelectContent>
                        {mediaInfo.qualities.map((q, idx) => (
                          <SelectItem key={idx} value={q.url}>{q.label}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </div>
              ...
              <div className="space-y-3 mt-6">
                {mediaInfo.videoUrl && (
                  <Button className="w-full h-12 text-base font-bold shadow-md shadow-primary/10"
                    onClick={() => handleDownload(selectedQuality || mediaInfo.videoUrl!, "video")}>
      ...
      {!mediaInfo && !loading && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-in fade-in slide-in-from-bottom-2 duration-700 delay-150">
          {[
            { icon: <FileVideo size={24} className="text-[#1AB7EA]" />, title: "Vimeo Videos", desc: "Download high quality Vimeo videos." },
            { icon: <FileVideo size={24} className="text-[#0066DC]" />, title: "Dailymotion", desc: "Save videos from Dailymotion." },
            { icon: <Download size={24} className="text-purple-500" />, title: "Quality Selection", desc: "Choose video resolution when available." }
          ].map((f, i) => (
