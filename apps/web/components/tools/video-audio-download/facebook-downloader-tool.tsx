...
import { FacebookLogo, Download, Link, Spinner, FileVideo, FileImage, Warning, ArrowClockwise } from "@phosphor-icons/react";
...
export function FacebookDownloaderTool() {
  ...
  const handleFetchInfo = async () => {
    if (!url) { toast.error("Please enter a Facebook URL"); return; }
    if (!url.includes("facebook.com") && !url.includes("fb.watch") && !url.includes("fb.com")) {
      toast.error("Please enter a valid Facebook URL"); return;
    }
    setLoading(true); setMediaInfo(null); setErrorMsg(null);
    try {
      const res = await fetch("/api/video-audio-download/facebook", {
  ...
  const handleDownload = (mediaUrl: string, type: "video" | "image") => {
    const fn = `facebook-${type}-${Date.now()}`;
  ...
  return (
    <div className="space-y-6">
      <Card className="border-border/60 shadow-sm overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#1877F2] via-[#4267B2] to-[#3578E5]" />
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <FacebookLogo className="h-6 w-6 text-[#1877F2]" weight="bold" />Facebook Downloader
          </CardTitle>
          <CardDescription>Download videos from Facebook by pasting the video link below.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              <Link className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input placeholder="https://www.facebook.com/watch?v=..." value={url}
            ...
            <Button onClick={handleFetchInfo} disabled={loading || !url}
              className="bg-gradient-to-r from-[#1877F2] to-[#3578E5] hover:opacity-90 transition-opacity">
          ...
          <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground bg-muted/30 p-2 rounded-lg border border-border/40">
            <Warning size={14} className="text-amber-500 shrink-0" /><span>Works for public videos only. Private or group videos are not supported.</span>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
            <div className="relative aspect-video bg-black flex items-center justify-center">
              {mediaInfo.type === "video" && mediaInfo.videoUrl ? (
                ...
                <><img src={`/api/video-audio-download/proxy?url=${encodeURIComponent(mediaInfo.imageUrl)}&filename=preview`}
                  alt="Facebook preview" className="w-full h-full object-contain" />
            ...
            <div className="p-6 flex flex-col justify-between">
              <div className="space-y-4">
                ...
                <div className="flex justify-between text-sm py-2 border-b border-border/40">
                  <span className="text-muted-foreground">Source</span><span className="font-medium">Facebook</span>
                </div>
              </div>
      ...
      {!mediaInfo && !loading && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-in fade-in slide-in-from-bottom-2 duration-700 delay-150">
          {[
            { icon: <FileVideo size={24} className="text-blue-500" />, title: "FB Videos", desc: "Download Facebook videos in HD." },
            { icon: <FileImage size={24} className="text-indigo-500" />, title: "Reels & Stories", desc: "Save Facebook Reels and public stories." },
            { icon: <Download size={24} className="text-sky-500" />, title: "Fast Download", desc: "Instant download via proxy server." }
          ].map((f, i) => (
